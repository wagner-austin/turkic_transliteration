...
\begin{tabular}{@{}llll@{}}
\toprule
Lang.\ (ISO)  & Script          & Phon.\ source                       & Symbols \\
\midrule
tr            & Latin           & \citet{zimmer1992turkish}           & 26      \\
az            & Latin           & \citet{mokari2017azerbaijani}       & 30      \\
kk            & Cyrillic        & \citet{mccollum2021kazakh}          & 37      \\
ky            & Cyrillic        & \citet{mccollum2020vowel}           & 37      \\
uz-lat        & Latin           & \citet{ido2025uzbek}                & 28      \\
uz-cyr        & Cyrillic        & \citet{ido2025uzbek}                & 28      \\
ug            & Arabic          & \citet{mccollum2021transparency}    & 33      \\
fi            & Latin           & \citet{suomi2009finnish}            & 26      \\
\bottomrule
\end{tabular}
\caption{The eight IPA rule files, their input orthography, and phonological source. \emph{Symbols} counts the distinct base characters the rule file can emit, taken over the right-hand sides of its rewrite rules with length marks, tie bars, and other diacritics excluded; it is a property of the rule file, recomputable from the released rules, and is not a claim about the language's phoneme inventory. Uzbek is covered in two rule files, one per orthographic input.}
...
\textbf{Corpus download.} The \texttt{turkic-download-corpus} CLI streams per-language slices of OSCAR-2301 directly from its Zstandard-compressed shards \citep{abadji-etal-2022-towards} — the Open Super-large Crawled Aggregated coRpus. OSCAR-2301 is Common Crawl web text after language classification and deduplication; it is the largest freely-licensed general-domain source for our target languages.

\textbf{Language-ID filtering.} Raw OSCAR slices contain non-trivial cross-language contamination, particularly in Central Asian Turkic sub-corpora where Russian-language content is common. The package scores each line's per-language probability with a FastText classifier \citep{joulin2016bag, joulin2017bag} and drops lines below a per-language threshold, by default $p \geq 0.95$ for the target language. The released CLI resolves the compact 176-language \texttt{lid.176} model, which deploys cheaply at corpus scale and is the standard first-pass filter in low-resource language work.%
% CO-AUTHOR NOTE: the corpora used in this paper were filtered with NLLB
% lid218e, not lid.176 — see the note at the end of the Training Datasets
% section. Reconcile both sections together before submission.
